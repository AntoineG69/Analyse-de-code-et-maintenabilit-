    ...
    void populateViews() {
        ...
        try {
            ...
            if (tag != null) {
                ...
                albumArtistName = getFirstFieldValue(tag, FieldKey.ALBUM_ARTIST);
                genre = tag.getFirst(FieldKey.GENRE);
                year = tag.getFirst(FieldKey.YEAR);
                track = tag.getFirst(FieldKey.TRACK);
                trackTotal = getFirstFieldValue(tag, FieldKey.TRACK_TOTAL);
                disc = getFirstFieldValue(tag, FieldKey.DISC_NO);
            }
        } catch (IOException | TagException | ReadOnlyFileException | InvalidAudioFrameException | UnsupportedOperationException ignored) {
            return null;
        }
